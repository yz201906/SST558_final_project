...
variables <- noquote(c("age_group", "Laterality", "Race_recode_W_B_AI_API", "Grade"))
...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        sidebarPanel(width = 3,
                     ...
                     selectInput('data_year', 'Year of data:', choices = unique(seer18_sub$Year_of_diagnosis), selected = "2005"),
...
server <- function(input, output, session) {
    ...
    train_index <- reactive({
        train_index <- createDataPartition(sample_train_data()$Survival_months,
                                           p = .7,
                                           list = FALSE,
                                           times = 1)
    ...
    output$summary_title <- renderText(paste0("Descriptive summary statistics for months survived in ", input$data_year))
    ...
    output$train_data_table <-
        renderDataTable(sample_train_data(), class = "display nowrap compact", caption="Table displayed shows only 50000 randomly sampled rows.", options = list(pageLength = 20), filter = "top")
    ...
    bar_plot_data <- reactive({
        if (input$sex == FALSE){
            plot_vars <- year_data() %>% select(Survival_months, input$selected_variable) 
    ...
    output$cv_fold_slider <- renderUI({
        if (input$trainMethod=="bagTree") {
            sliderInput('cv_fold', "Adjust # of CV folds", min = 5, max = 15, step = 1, value = 10)
        }
    })
    output$cv_repeat_slider <- renderUI({
        if (input$trainMethod=="bagTree") {
            sliderInput('cv_repeat', "Adjust # of CV repeats", min = 1, max = 5, step = 1, value = 3)
        }
    })
    ...
    mlr_trained <- eventReactive(input$train_model_sup, {
        if (isolate(input$trainMethod)=="mlr") {
            ...
            linear_model <-step((linear_model_fit), direction = "both", trace = 0)
        }
    })
    observeEvent(input$train_model_sup, {
    ...
    bt_trained <- eventReactive(input$train_model_sup,{
        if (isolate(input$trainMethod)=="bagTree") {
            ...
            bt_fit <- train(Survival_months ~ ., isolate(recoded_data()), method = "gbm", verbose = 0,
                            preProcess=c("center", "scale"), trControl = trainControl(method = "repeatedcv", number = isolate(input$cv_fold), repeats = isolate(input$cv_repeat), allowParallel = TRUE, verboseIter = FALSE),
                            tuneGrid = expand.grid(n.trees = isolate(input$n_tree), interaction.depth = c(1,4,9),
    ...
    # Prediction
    output$test_data_table <- renderDataTable(test_data(), class = "display nowrap compact", caption="Please select a row for prediction.", options = list(pageLength = 5, sDom  = '<"top">lrt<"bottom">ip'), selection = 'single', filter = "top")
    ...
    selected_test_data <- eventReactive(input$test_data_table_rows_selected, {
        my_data <- test_data() %>% select(!Survival_months)
        my_data <- encode_data(my_data) %>% slice(input$test_data_table_rows_selected)
        as.data.frame(my_data)
    })
    observeEvent(input$test_data_table_rows_selected, {
        output$pred_result <- renderText({
            if (isolate(input$trainMethod)=="mlr") {
                ...
                paste0("The predicted number of months of survival based on selected case is: ", round(months,digits = 0))
            } else {
